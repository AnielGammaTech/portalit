...
Deno.serve(async (req) => {
  try {
    ...
    if (!JUMPCLOUD_API_KEY || !JUMPCLOUD_PROVIDER_ID) {
      return Response.json({ success: false, error: 'JumpCloud credentials not configured' });
    }
    ...
    if (!allMappings || allMappings.length === 0) {
      return Response.json({ success: true, message: 'No JumpCloud mappings found', synced: 0 });
    }
